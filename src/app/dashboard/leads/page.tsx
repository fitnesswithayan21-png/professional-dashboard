    ...
    return (
      <div className="flex flex-col gap-2.5">
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">AI Insight</span>
        </div>
        
        <div className="flex flex-col gap-2 mt-1">
          <div className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-[11.5px] font-semibold w-fit", qualTint)}>
            {qualIcon} {qualText}
          </div>
          <div className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-[11.5px] font-semibold w-fit", intentTint)}>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-[640px]">
        ...
        <div className="h-[88px] bg-white border border-[#E2E8F0] rounded-[12px] p-[16px] flex flex-col justify-between items-center text-center shadow-[0_1px_2px_rgba(0,0,0,0.02)] hover:shadow-md transition-all duration-200">
          <div className="flex items-center justify-center gap-1.5">
            <span className="text-[10px] font-bold text-slate-450 uppercase tracking-wider leading-none">NEW TODAY</span>
          ...
          <div className="flex items-center gap-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-[10px] p-1 h-11">
            <button
              ...
              className={cn(
                ...
                activeView === 'table' && "bg-white text-slate-900 shadow-sm border border-[#E2E8F0]"
              )}
            >
            ...
            <button
              ...
              className={cn(
                "h-9 px-4 rounded-md text-slate-500 hover:text-slate-900 transition-all cursor-pointer flex items-center gap-1.5 text-[12.5px] font-medium", 
                activeView === 'grid' && "bg-white text-slate-900 shadow-sm border border-[#E2E8F0]"
              )}
            >
          ...
          <div className="overflow-x-auto px-1">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center py-12 text-[13px] text-slate-500">No leads found matching filters.</td>
                  </tr>
                ) : (
                  paginated.map((lead) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-[20px] py-2">
                          <div className="flex flex-col">
                            <span className="text-[13px] font-medium text-slate-700 leading-tight">{lead.businessType || '—'}</span>
                            <span className="text-[11px] text-slate-400 capitalize font-normal mt-0.5">{lead.source}</span>
                          </div>
                        </td>
                        <td className="px-[20px] py-2">
            ...
            >
              <div className="p-6 flex flex-col h-full">
                ...
                <div className="flex items-start gap-4 mb-6">
                  <Avatar name={lead.fullName} className="h-12 w-12 text-[15px] font-bold shrink-0 shadow-sm" />
                  ...
                  <div className={cn("w-2.5 h-2.5 rounded-full shrink-0 mt-2", lead.status.toLowerCase() === 'new' ? 'bg-purple-500' : lead.status.toLowerCase() === 'qualified' ? 'bg-emerald-500' : 'bg-slate-300')} />
                </div>

                {/* Middle Section: AI Intelligence Module */}
                <div className="mt-auto mb-6">
                   {renderAIIntelligence(lead)}
                </div>
                ...
                <div className="flex items-center justify-between mt-2 pt-5 border-t border-slate-100/80">
                   <div className="flex flex-col gap-0.5">
                     <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest">Activity Timeline</span>
                     <span className="text-[12px] font-medium text-slate-600">Active {formatDate(lead.lastContactTime || lead.createdDate)} • Created {formatDate(lead.createdDate)}</span>
                   </div>
                </div>
              </div>
              ...
              <div className="h-[48px] bg-slate-50 border-t border-slate-200/60 flex items-center px-2 group-hover:bg-slate-100/50 transition-colors">
                ...
                <button className="flex-1 flex items-center justify-center gap-1.5 h-full text-slate-400 hover:text-blue-600 transition-colors" title="AI Memory">
                  <Brain className="h-4 w-4" />
                </button>
                <div className="w-px h-6 bg-slate-200" />
                <button className="flex-1 flex items-center justify-center gap-1.5 h-full text-slate-400 hover:text-emerald-600 transition-colors" title="Schedule">
                  <Calendar className="h-4 w-4" />
                </button>
                <div className="w-px h-6 bg-slate-200" />
                <button className="flex-1 flex items-center justify-center gap-1.5 h-full text-slate-400 hover:text-indigo-600 transition-colors" title="Message">
        ...
        {selectedLead && (
          <div className="flex flex-col h-full bg-[#F8FAFC] text-slate-800 font-sans">
            ...
            <div className="px-6 py-4 flex justify-between items-center bg-white border-b border-slate-100">
              <span className="text-[14px] font-semibold text-slate-800">Lead Details</span>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 custom-scrollbar">
              ...
              <div className="flex flex-col items-center gap-3">
                <Link href={`/dashboard/conversations?leadId=${selectedLead.id}`} className="block w-full max-w-[330px]">
                  <div className="relative flex items-center justify-center h-[52px] px-4 bg-white border border-slate-200/60 rounded-[14px] hover:border-blue-300 hover:shadow-[0_4px_12px_rgba(0,0,0,0.05)] transition-all group cursor-pointer">
                    ...
                    <span className="text-[14px] font-semibold text-slate-700 group-hover:text-blue-700 transition-colors">View Conversations</span>
                ...
                <Link href={`/dashboard/ai-memory?leadId=${selectedLead.id}`} className="block w-full max-w-[330px]">
                  <div className="relative flex items-center justify-center h-[52px] px-4 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-[14px] hover:shadow-[0_4px_16px_rgba(99,102,241,0.25)] transition-all group cursor-pointer hover:-translate-y-[1px]">
                    ...
                    <span className="text-[14px] font-semibold text-white">View AI Memory</span>
                ...
                <Link href={`/dashboard/appointments?leadId=${selectedLead.id}`} className="block w-full max-w-[330px]">
                  <div className="relative flex items-center justify-center h-[52px] px-4 bg-white border border-slate-200/60 rounded-[14px] hover:border-blue-300 hover:shadow-[0_4px_12px_rgba(0,0,0,0.05)] transition-all group cursor-pointer">
                    ...
                    <span className="text-[14px] font-semibold text-slate-700 group-hover:text-blue-700 transition-colors">Schedule Appointment</span>
                    <ChevronRight className="absolute right-4 h-4 w-4 text-slate-300 group-hover:text-blue-500 transition-all group-hover:translate-x-0.5" />
                  </div>
                </Link>
              </div>
              ...
              <div className="bg-white border border-slate-200/60 rounded-[16px] p-6 shadow-sm">
                <h4 className="text-[13px] font-bold text-slate-900 tracking-tight mb-4">Lead Snapshot</h4>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-emerald-50/40 border border-emerald-100/60 rounded-[12px] p-3.5 h-[72px] flex flex-col items-center justify-center text-center gap-1.5 transition-colors hover:bg-emerald-50/70 shadow-[0_1px_2px_rgba(0,0,0,0.01)]">
                    <span className="text-[11px] font-semibold text-emerald-500/80 uppercase tracking-wider leading-none">Status</span>
                    <div className="flex"><Badge status={selectedLead.status} className="px-2.5 py-0.5 text-[11px] leading-none" /></div>
                  </div>
                  {/* Lead Score */}
                  <div className="bg-amber-50/40 border border-amber-100/60 rounded-[12px] p-3.5 h-[72px] flex flex-col items-center justify-center text-center gap-1.5 transition-colors hover:bg-amber-50/70 shadow-[0_1px_2px_rgba(0,0,0,0.01)]">
                    <span className="text-[11px] font-semibold text-amber-500/80 uppercase tracking-wider leading-none">Lead Score</span>
                    <span className="text-[16px] font-black text-amber-700 tracking-tight leading-none">{selectedLead.leadScore}</span>
                  </div>
                  {/* Created Date */}
              ...
              <div className="bg-white border border-slate-200/60 rounded-[16px] overflow-hidden shadow-sm flex flex-col mb-4">
                {/* Header Section */}
                <div className="py-6 pr-6 border-b border-slate-100 bg-slate-50/50" style={{ paddingLeft: '40px' }}>
                  <div className="flex items-center gap-2 mb-1">
                    <Brain className="h-5 w-5 text-indigo-600" />
                    <h3 className="text-[18px] font-bold text-slate-900 tracking-tight">Lead Intelligence</h3>
                  </div>
                  <p className="text-[13px] text-slate-500 font-medium">
                    AI-powered analysis of CRM activity, qualification signals, and engagement history.
                  </p>
                </div>
                ...
                <div className="p-6 flex flex-col gap-8">
                  ...
                  <div className="flex flex-col md:flex-row gap-6 items-center">
                    ...
                    <div className="flex-shrink-0 flex flex-col items-center justify-center p-2" style={{ marginLeft: '40px' }}>
                      <div className="relative w-24 h-24 flex items-center justify-center mb-3">
                        <svg className="w-full h-full -rotate-90 drop-shadow-sm" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
