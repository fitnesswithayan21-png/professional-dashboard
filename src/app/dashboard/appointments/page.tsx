...
export default function AppointmentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return result.sort((a, b) => {
      const ta = a.appointmentStart ? new Date(a.appointmentStart).getTime() : new Date(a.appointmentDate).getTime();
      const tb = b.appointmentStart ? new Date(b.appointmentStart).getTime() : new Date(b.appointmentDate).getTime();
      return ta - tb;
    });
  ...
  const renderAction = (apt: typeof enriched[number], compact = false) => {
    ...
    if (status === 'scheduled' || status === 'confirmed') {
      return (
        <a
          ...
          className={cn(btnBase, 'bg-[#2563EB] hover:bg-blue-700 text-white shadow-[0_4px_12px_rgba(37,99,235,0.2)] hover:shadow-[0_6px_16px_rgba(37,99,235,0.3)] hover:-translate-y-0.5')}
        >
    ...
    if (status === 'completed') {
      return (
        <div className={cn(btnBase, 'bg-emerald-50 text-emerald-700 border border-emerald-100/50 cursor-default')}>
    ...
    if (status === 'no-show') {
      return (
        <div className={cn(btnBase, 'bg-rose-50 text-rose-700 border border-rose-100/50 cursor-default')}>
    ...
    if (status === 'cancelled') {
      return (
        <div className={cn(btnBase, 'bg-slate-50 text-slate-500 border border-slate-200/60 cursor-default')}>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="flex items-center gap-1 shrink-0 bg-white/80 backdrop-blur-md border border-slate-200/60 rounded-[14px] p-1 shadow-sm">
            <button
              ...
              className={cn('p-2 rounded-[10px] transition-all duration-200 cursor-pointer', view === 'calendar' ? 'bg-white text-slate-900 shadow-sm scale-105' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-200/30')}
              title="Grid View"
            >
            ...
            <button
              ...
              className={cn('p-2 rounded-[10px] transition-all duration-200 cursor-pointer', view === 'agenda' ? 'bg-white text-slate-900 shadow-sm scale-105' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-200/30')}
              title="Agenda View"
            >
      ...
      {view === 'calendar' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 relative z-10">
          {filtered.map(apt => {
            ...
            const dispDate = formatDisplayDate(apt.appointmentDate || apt.appointmentStart || '');
              ...
              >
                <div className="flex-1 p-6 flex flex-col h-full">
                  ...
                  <div className="flex items-start gap-4 mb-5">
                    <Avatar name={name} className="h-11 w-11 text-[14px] font-semibold shadow-sm shrink-0" />
                    <div className="flex flex-col min-w-0">
                      ...
                      {source && (
                        <span className="text-[12px] text-slate-400 font-medium capitalize">{source}</span>
                      )}
                      <div className="mt-1.5">
                        <Badge status={effStatus} className="rounded-[6px] px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase border-none" />
                      </div>
                    </div>
                  </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-5">
                    <Avatar name={name} className="h-11 w-11 shadow-sm font-semibold shrink-0" />
                    <div>
                      <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                        <h4 className="text-[16px] font-bold text-slate-900 tracking-tight group-hover:text-[#2563EB] transition-colors">{name}</h4>
                        <Badge status={effStatus} className="rounded-[6px] px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase border-none" />
                      </div>
                      {source && <p className="text-[12px] text-slate-400 font-medium capitalize mb-1">{source}</p>}
