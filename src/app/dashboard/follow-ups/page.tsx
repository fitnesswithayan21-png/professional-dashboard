...
export default function FollowUpsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(f => f.leadName.toLowerCase().includes(s) || f.followUpMessage.toLowerCase().includes(s));
    }
    if (statusFilter !== 'all') {
  ...
  const stats = useMemo(() => {
    ...
    const responseRate = total > 0 
      ? Math.round((followUps.filter(f => f.responseReceived).length / total) * 100) 
      : 0;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-3">
        <p className="text-[13px] text-slate-500">Monitor automated sequences, delivery states, and dispatch outbound outreach campaigns.</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-white border border-slate-200">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Queue</p>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-2xl font-bold text-slate-900">{stats.total}</span>
            <span className="text-[11px] text-blue-600 font-semibold uppercase">100% Automated</span>
          </div>
        </Card>

        <Card className="bg-white border border-slate-200">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Outbox Pending</p>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-2xl font-bold text-slate-900">{stats.pending}</span>
            <span className="text-[11px] text-amber-600 font-semibold uppercase">Auto-dispatching</span>
          </div>
        </Card>

        <Card className="bg-white border border-slate-200">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Delivered Outreach</p>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-2xl font-bold text-emerald-600">{stats.completed}</span>
            <span className="text-[11px] text-emerald-600 font-semibold uppercase">99.8% Success</span>
          </div>
        </Card>
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-white p-2 rounded-xl border border-slate-200 shadow-xs">
            <div className="flex gap-1 overflow-x-auto custom-scrollbar w-full sm:w-auto">
              {['all', 'pending', 'sent', 'completed', 'failed'].map(status => (
                <button 
                  key={status} 
                ...
                >
                  ...
                  <span className={cn(
                    ...
                    {status === 'all' ? followUps.length : followUps.filter(f => f.status === status).length}
                  </span>
                </button>
              ))}
            </div>
              ...
              filtered.map((fu) => (
                <div key={fu.id} className="relative group">
                  <div className="absolute -left-[27px] top-1.5 z-10">
                    <Avatar name={fu.leadName} size="sm" className="ring-4 ring-[#F8FAFC] shadow-sm border border-slate-200" />
                  </div>
                  ...
                  <Card className="bg-white border border-slate-200 p-4 relative overflow-hidden group hover:border-slate-350 transition-all duration-200">
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      <div className="space-y-1.5 flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <h4 className="text-[13px] font-semibold text-slate-900 group-hover:text-blue-600 transition-colors">{fu.leadName}</h4>
                          <span className="text-[11px] text-slate-400 uppercase font-bold tracking-wider">Step #{fu.followUpNumber}</span>
                          <span className="text-[10px] font-mono text-slate-400">#{fu.id}</span>
                        </div>
                        <p className="text-[13px] text-slate-650 leading-relaxed font-light">{fu.followUpMessage}</p>
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 border-t border-slate-100 pt-3 mt-3 text-[10px] font-bold uppercase tracking-wider">
                      <div className={cn(
                        ...
                        fu.status === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                        fu.status === 'failed' ? 'bg-rose-50 text-rose-700 border-rose-200' :
                      ...
                      <div className={cn(
                        "flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[9px]",
                        fu.messageSent ? 'bg-emerald-55 text-emerald-700 border-emerald-200' : 'bg-slate-50 text-slate-400 border-slate-200'
                      )}>
                        <CheckCheck className="h-3 w-3" />
                        <span>Delivery: {fu.messageSent ? 'Sent' : 'Pending'}</span>
                      </div>

                      <div className={cn(
                        "flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[9px]",
                        fu.responseReceived ? 'bg-blue-50 text-blue-600 border-blue-200 animate-pulse' : 'bg-slate-50 text-slate-400 border-slate-200'
                      )}>
                        <MessageSquare className="h-3 w-3" />
                        <span>Engagement: {fu.responseReceived ? 'Replied' : 'No reply'}</span>
                      </div>
                    </div>
                  </Card>
                </div>
        ...
        <div className="lg:col-span-4 space-y-5">
            ...
            <div className="space-y-4">
              <div className="p-3 rounded bg-slate-50 border border-slate-200 space-y-2">
                ...
                <p className="text-[11px] text-slate-500 font-light leading-relaxed">
                  "Hi {"{Name}"}, noticed {"{Company}"} is scaling virtual workflows. Our AI agent qualifies leads automatically in real-time. Can I share a quick 2-min demo link?"
                </p>
              ...
              <div className="p-3 rounded bg-slate-50 border border-slate-200 space-y-2">
                ...
                <p className="text-[11px] text-slate-500 font-light leading-relaxed">
                  "Hey {"{Name}"}, follow-up on my email. NexusAI overrides CRM objection templates automatically to save 30 hrs/week. Do you have 10 mins this Wed?"
                </p>
                <div className="flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-wider pt-1.5">
                  <span>80% Sent</span>
              ...
              <div className="p-3 rounded bg-slate-50 border border-slate-200 space-y-2 opacity-65">
                ...
                <p className="text-[11px] text-slate-400 font-light leading-relaxed">
                  "Name, closing this ticket for now. If you ever want to automate sheet telemetry workflows, feel free to reactivate."
                </p>
              </div>
            </div>
          ...
          <Card className="bg-white border border-slate-200 p-4 space-y-3.5">
            ...
            <p className="text-[11.5px] text-slate-500 leading-relaxed">
              Trigger instant CRM lead sequencing outbound alerts. Select active sheet lists and override default Grok templates.
            </p>
